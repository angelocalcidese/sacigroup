...
############################# inserisco documento da tabella tipo intervento #######################          
$numerotipoin="";
$cercasi=0;         
$sql1 = "SELECT * FROM tipointervento  where cliente = '$clientex' and commessa = '$commessax' and tipointervento1 = '$tipointervento' ";
#echo $sql1; 
$result = $conn->query($sql1);
if ($result->num_rows > 0) {
  while($macrogruppo = $result->fetch_assoc())
		{ $numerotipoint = $macrogruppo["tipointervento"];
        #echo "num ".$numerotipoint."<br>";	 
			} }           
$sql1h = "SELECT * FROM documenti  where cliente = '$clientex' and commessa = '$commessax' and tipodoc = '$numerotipoint' and siticket = 'SI'";
#echo $sql1h ; 
$resulth = $conn->query($sql1h);
if ($resulth->num_rows > 0) {
  while($macrogruppoh = $resulth->fetch_assoc())
		{ $prgr = $macrogruppoh["progr"];
   $data = $macrogruppoh["datadoc"];
   $datasca = $macrogruppoh["datasca"];
   $tipodoc = $macrogruppoh["tipodoc"];
   $oggettox= $macrogruppoh["oggetto"];
   $file = $macrogruppoh["file"];
   $siticket = $macrogruppoh["siticket"];
$sqlb = "INSERT INTO documenti
              (               
              tipodoc, 
              datadoc, 
              oggetto, 
...
#echo $sql; exit;
  if ($conn->query($sqlb) === TRUE)
        ...
        else { echo $sqlb."Errore inserimento recoerd: " . $conn->error; } 
} }         
########################  fine inserimento documento a corredo ticket #####################                                        
} else
{
...
#echo $sql; exit;
  if ($conn->query($sqlr) === TRUE)
        ...
        '$numero',
        '$dataoggi',
        'Memorizzato nuovo Assegnamento a 3PP',
        '$login',
        '',
        '',
        '')";
...
<!--        <input class="required" type="text" name="apparato" id="apparato" onkeyup="ricercaAutocompleteApparato()" value="<?php echo $apparato; ?>" maxlength="200" size="30" > -->
      ...
      <td colspan="1">
        <div style="width:49%; float:left;" >
          ...
          <input type="text"  name="seriale" id="seriale" value="<?php echo $seriale; ?>" maxlength="200" size="12" >
        </div>
