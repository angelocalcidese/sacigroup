...
$sql = "SELECT *  FROM  gite where  progr = '1' "; 
...
if ($result->num_rows > 0) {
  ...
  $newimmaggine="img".$numero.$estensione;
   $sql = "UPDATE `gite` SET `immagine`='$newimmaggine'  WHERE `progr` = '1' ";   
    if ($conn->query($sql) === FALSE)
        {echo "errore ".$sql."<br>";} 
          else
        {}     

$myfile = fopen("./gite/poma1.txt", "w") or die("Unable to open file!");
$txt = $newimmaggine."\n";
fwrite($myfile, $txt);
$txt = $newimmaggine."\n";
fwrite($myfile, $txt);
...
?>
<script>
window.location.href='caricagite.php?login=<? echo $login; ?>';
</script>
<?php
}
...
<div id="containerx" align="left"> 
      <td ><a href="menugenerale.php?login=<?php echo $login; ?>">Servizi</a>/Gite(sito)</td></tr>
</div>
</div>
<br>
<div align="center">   
<div id="container">
<p align="center"><b><font face="Arial" size="6" color="#993300">CARICA GITE IN SITO</font></b></p>
...
$file="gite/".$immagine; //ricavo da db il nome dell'inmmagine e gli do il percorso
...
?>
<!-- <p style="text-align: center;"><img src="https://www.spi.it/poma/gite/poma1.jpg?<?=time()?>" align="absmiddle" style="100%;height: 100%;" /></p>   -->
</div>
</div>
