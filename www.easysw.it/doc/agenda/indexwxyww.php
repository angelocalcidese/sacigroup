...
where numero = '$numcanc1'";
#echo  $sql; exit;
if ($conn->query($sql) === TRUE)
    { } 
    else { echo "Error inserted record: " . $conn->error; } 
$sql = "DELETE from prenotagruppo where prenotazione = '$numcanc1' ";
#echo  $sql; exit;
if ($conn->query($sql) === TRUE)
    { } 
    else { echo "Error inserted record: " . $conn->error; } 
...
include "conf_DB.php";
$sql = "DELETE from prenotazioni where numero = '$numcanc' ";
#$sql = "UPDATE prenotazioni set chidataannulla = CURRENT_TIMESTAMP,
#loginannulla = '$login'
...
function ShowCalendar($m,$y,$zx)
{
  ...
  . "&zx="
  . $zx.
  "&add=0\"><img src=\"sinistro.png\" width=\"45\" height=\"40\"></a>
  " . "<font face=\"Arial\" size=\"6\" color=\"#993300\">".$nomi_mesi[$m-1] . " " . $y ."</font>". " 
  <a href=\"?d=" . $successivo  
  . "&zx="
  . $zx.
  "&add=0\"><img src=\"destro.png\" width=\"45\" height=\"40\"></a></td></font></tr></div>";
  ?>
  <div align="center">
  ...
  while($macrogruppox = $resultx->fetch_assoc()) 
    ...
    $sw2='<font size="2">';
      ...
      <? if ($colore== "#f61f24"  or $colore== "#07e837" or $colore== "#c19bdf"){ ?>
       <a href="JavaScript:carica_consegnaC('../../arrivoxx.php?tessera=<?php echo $numerox; ?>');">
       <? }else { 
       if ($colore== "#faf82a"){  ?>
      ...
      ."<br>Prenotato da: ".$dataentrataee." a: ".$datafinexxyys."<br>".
      $messaggio    
      ;?></font>
      <? if ($colore== "#f61f24"  or $colore== "#07e837" or $colore== "#c19bdf"){ ?>
      <br>
      ...
      <input type="hidden" name="numero1" value="<? echo $numerox; ?>" />
      <button type="submit">CANCELLA</button></form>
      <? } ?>
      ...
      <br>
       ...
       <font face="Arial" color="#000000"><b>PRENOTAZIONE CONFERMATA?</b></font>
					...
					<option <? if($confermatods=="SI"){echo "selected";}?>>SI</option>
					<option <? if($confermatods=="NO"){echo "selected";}?>>NO</option>
					</select>
      <input type="hidden" name="ingranaggio" value="2" />
      <input type="hidden" name="numero2" value="<? echo $numerox; ?>" />
      <button type="submit">MODIFICA</button></form></form>
      <? } ?>
      <br>
      </span>
      </div>
   ...
   $datarr = (date($day."/".$m."/".$y)); ?>
   <a href="JavaScript:carica_consegnaB('../../prenotaxx.php?data=<?php echo $datarr; ?>');"><font color="#ffffff" >aa</font></a>
...
?>
<div align="center">
   <p><a href="?d=<? echo $dx; ?>&zx=<? echo $zx; ?>&add=10"><font face="Arial" size="4" color="#993300">Zoom +&nbsp;&nbsp;&nbsp;</font></a>
      <a href="?d=<? echo $dx; ?>&zx=<? echo $zx; ?>&add=-10"><font face="Arial" size="4" color="#993300">Zoom -</font></a>
      </p>
  </div>  
</div> 
...
function carica_consegnaB(url){
	<!-- intestazione -->	
    popupWindow =          
		window.open(url,'pdf','width=850,height=650,left=150,top=150,location=0,directories=0,toolbar=0,scrollbars=yes')
    //	window.open('esponipdf1x.php?offerta='+offerta,'offerta','width=1200,height=800,left=150,top=150,location=0,directories=0,toolbar=0')
		//return false;
