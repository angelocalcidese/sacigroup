...
if ($lun!="")
   {$selezionalun=" and lun = 1 ";}
   else
   {$selezionalun="";}    
if ($mar!="")
   {$selezionamar=" and mar = 1 ";}
   else
   {$selezionamar="";}  
if ($mer!="")
   {$selezionamer=" and mer = 1 ";}
   else
   {$selezionamer="";}     
if ($gio!="")
   {$selezionagio=" and gio = 1 ";}
   else
   {$selezionagio="";}     
if ($ven!="")
   {$selezionaven=" and ven = 1 ";}
   else
   {$selezionaven="";}     
if ($dom!="")
   {$selezionadom=" and dom = 1 ";}
   else
   {$selezionadom="";}     
if ($mensa!="")
...
<head>
	...
	<title>Messa della Carità invio email massive</title>
...
if ($tessera!="")
   {$sql = "SELECT *  FROM  socivolontari where  tessera = '$tessera' " . " and email like '%@%'  and invio = 0 " .
    ...
    {$sql = "SELECT *  FROM  socivolontari where  tessera != '' " 
        . $selezionacognome . $selezionanome .  $selezionafrequenza . $selezionaemergenze . $selezionareferente .   
        $selezionamembro . $selezionalun . $selezionamar .
        $selezionamer . $selezionagio . $selezionaven .
        $selezionadom . $selezionamensa . $selezionaguardaroba .
        $selezionatesseramento . $selezionavarie. " and email like '%@%'  and invio = 0 " . 
        "order by cognome"; }  
#echo $sql; 
...
<p align="center"><b><font face="Arial" size="4" color="#993300">SELEZIONE VOLONTARI PER INVIO <? echo $totale; ?> EMAIL</font></b></p>
...
<table border="1" width="100%" bgcolor="#FFF4F7" class="table6" >	
  <tr>
  	...
  	<td><select size="1" name="mittente" style="background-color: #ececee">
         	<option <? if($mittente=="paolo.bechini@gmail.com"){echo "selected";}?>>paolo.bechini@gmail.com</option>
<!--					<option <? if($mittente=="g.fontaniello47@gmail.com"){echo "selected";}?>>g.fontaniello47@gmail.com</option> -->
					<option <? if($mittente=="operamessacarita@gmail.com"){echo "selected";}?>>operamessacarita@gmail.com</option>
          </select></td>
   </tr>
	<tr>
		<td><font size="3" face="Arial" color="#000000">Oggetto: </font>   </td>
    <td><input style="font-size: 20px;" name="oggetto"  size="63" value="<? echo $oggetto;?>"></td>
   </tr>
  <tr >
...
$mail = new PHPMailer();
$mail->From     = ($mittente);
$mail->FromName = "messaggio MESSA DELLA CARITA'";
